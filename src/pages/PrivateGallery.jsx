...
import { X, ChevronLeft, ChevronRight, Play, Image, Video, Loader, Lock, Upload } from "lucide-react";
...
export default function PrivateGallery() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      // Filter only private tagged resources
      const filterPrivate = (arr, type) =>
        arr
          .filter((r) => r.tags && r.tags.includes(PRIVATE_TAG))
          .map((r) => ({ ...r, media_type: type }))
          .sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
      
      setAllImages(filterPrivate(imgs, "image"));
      setAllVideos(filterPrivate(vids, "video"));
    } catch { setError("Could not load private gallery."); }
  ...
  useEffect(() => {
    const h = () => loadAll();
    window.addEventListener("private-gallery-refresh", h);
    return () => window.removeEventListener("private-gallery-refresh", h);
  }, [loadAll]);
  ...
  return (
    <div className="gallery-page private-gallery-page">
      {/* ── Private Header ── */}
      <div className="private-gallery-header">
        ...
        <NavLink to="/private/upload" className="private-upload-link">
          <Upload size={16} />
      ...
      {loading && (
        <div className="gallery-loading">
          <Loader size={32} className="spin" /><p>Loading your private gallery…</p>
        </div>
      )}
      ...
      {!loading && !error && allImages.length === 0 && allVideos.length === 0 && (
        <div className="gallery-empty">
          ...
          <p>No private media found. Upload some private photos or videos!</p>
          <NavLink to="/private/upload" className="private-empty-link">
